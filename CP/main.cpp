...
class SuffixTrie {
  ...
  public:
    ...
    /**
     * @brief Find most common part of 2 strings of view s1 + '$' + s2 + '#' where '$' and '#' are sentinels
     *
     * @param s1Size -- size of s1
     * @param s2Size -- size of s2
     *
     * @return len of most common part and index in s1 where it starts
     */
    MCSstate findMCS_impl(Node * current, size_t s1Size, size_t curSize, MCSstate & maxState) {
      MCSstate curState{false, false, curSize + current->getSliceSize(), current};
      if(current->suffixStartPos.has_value()) {
        std::cout << current->suffixStartPos.value() << std::endl;
        if(current->suffixStartPos.value() < s1Size)
         curState.first = true;
        else
          curState.second = true;
      }

      for(auto & [_, child] : current->translations) {
        MCSstate childState = findMCS_impl(child, s1Size, curSize + current->getSliceSize(), maxState);
        std::cout << childState.first << ' ' << childState.second << std::endl;
        curState.first = curState.first || childState.first;
        curState.second = curState.second || childState.second;
      }
    std::cout << "---------------------" << std::endl;

      if(curState.first && curState.second && curState.size >= maxState.size) {
        maxState.size = curState.size;
        maxState.node = current;
        std::cout << "Here: a = " << current->slice.first << " b = " << *current->slice.second << std::endl;
      }

      return curState;
    }

    std::pair<size_t, size_t> findMostCommonSlice(size_t s1Size, size_t s2Size) {
      MCSstate maxState = {true, true, 0, root_};
      findMCS_impl(root_, s1Size, 0, maxState);
      std::cout << maxState.size << std::endl;
      exit(0);
    };
};
...
void SuffixTrie::printTree(SuffixTrie::Node *node, size_t depth) {
  ...
  std::cout << node->suffixStartPos.value_or(0);
...
int main() {
  ...
  SuffixTrie trie(s3);
  trie.findMostCommonSlice(s1.size() + 1, s2.size() + 1);
